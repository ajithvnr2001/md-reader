...
- **⛶ 1-Click Distraction-Free Full Screen Content Reading Mode**: Toggle borderless full-screen reading via topbar button (`#fullScreenBtn`) or keyboard shortcut (`Ctrl+Shift+F`). Hides all UI chrome, topbar, sidebar, and toolbars, leaving ONLY smooth document scrolling, a reading-progress chip, and an **auto-fading Zoom (+/−) bar** (`A×` + `Fit`). Includes a **pseudo-fullscreen fallback** for browsers without the Fullscreen API (e.g. iOS Safari), **pinch-to-zoom** on touch, per-document **Auto-fullscreen preference**, and graceful `ESC` exit everywhere.
